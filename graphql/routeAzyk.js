...
const {tomtom, reductionSearchText, isNotEmpty, checkDate, isEmpty, dayStartDefault, defaultLimit} = require('../module/const');
...
const query = `
    ...
    route(_id: ID!): Route
    unloadingInvoicesFromRouting(orders: [ID]!, organization: ID!): String
`;

const mutation = `
    buildRoute(provider: ID!,autoTonnage: Int!, length: Int, orders: [ID]!): [Delivery],
    addRoute(deliverys: [DeliveryInput]!, provider: ID!, selectProdusers: [ID]!, selectDistricts: [ID]!, selectEcspeditor: ID!, selectAuto: ID!, selectedOrders: [ID]!, dateDelivery: Date!, allTonnage: Int!): String,
    setRoute(route: ID!, deletedOrders: [ID]!): String,
    deleteRoute(_id: ID!, selectedOrders: [ID]!): String
`;

const resolvers = {
    unloadingInvoicesFromRouting: async(parent, {orders, organization}, {user}) => {
        if([roleList.admin, 'агент', 'суперорганизация', 'организация', 'менеджер', 'экспедитор', 'суперэкспедитор'].includes(user.role)) {
            ...
            for(let i = 0; i<data.length;i++) {
                for(let i1 = 0; i1<data[i].orders.length;i1++) {
                    ...
                    if(isEmpty(allCount[`${data[i].organization._id}|||${data[i].organization.name}`])) {
                    ...
                    for(let i1 = 0; i1 < data[i].adss.length; i1++) {
                        if(data[i].adss[i1].item) {
                            ...
                            if (data[i].adss[i1].multiplier) {
                                ...
                                else if (data[i].adss[i1].targetType === 'Товар' && data[i].adss[i1].targetItems && data[i].adss[i1].targetItems.length) {
                                    let index = []
    ...
    listDownload: async(parent, {orders}, {user}) => {
        if([roleList.admin, 'агент', 'суперорганизация', 'организация', 'суперэкспедитор', 'экспедитор', 'менеджер'].includes(user.role)) {
                    ...
                    for(let i1 = 0; i1 < orders[i].adss.length; i1++) {
                        if(orders[i].adss[i1].item) {
                            ...
                            if (orders[i].adss[i1].multiplier) {
                                ...
                                else if (orders[i].adss[i1].targetType === 'Товар' && orders[i].adss[i1].targetItems && orders[i].adss[i1].targetItems.length) {
                                    let index = []
    ...
    routes: async(parent, {organization, search, sort, filter, date, skip}, {user}) => {
        if(['суперорганизация', 'организация', 'менеджер', 'агент', roleList.admin, 'экспедитор', 'суперэкспедитор'].includes(user.role)) {
            let dateStart;
            let dateEnd;
            if(date) {
    ...
    route: async(parent, {_id}, {user}) => {
        if(mongoose.Types.ObjectId.isValid(_id)) {
            let route = await RouteAzyk.findById(_id)
                .populate({
                    path: 'selectEcspeditor',
                    select: 'name _id'
               })
...
const resolversMutation = {
    ...
    addRoute: async(parent, {deliverys, provider, selectProdusers, selectDistricts, selectEcspeditor, selectAuto, selectedOrders, dateDelivery, allTonnage}, {user}) => {
        if([roleList.admin, 'агент', 'суперорганизация', 'организация', 'менеджер'].includes(user.role)) {
            ...
            await Promise.all([
                RouteAzyk.create({
                    deliverys, provider: provider!=='super'?provider:null, selectProdusers, selectDistricts: selectDistricts.filter(selectDistrict=>selectDistrict!=='super'),
                    selectEcspeditor, selectAuto, selectedOrders, dateDelivery, status: 'Cоздан', number, allTonnage
               }),
                InvoiceAzyk.updateMany({_id: {$in: selectedOrders}}, {distributed: true})
            ])
       }
        return 'OK';
   },
    deleteRoute: async(parent, {_id, selectedOrders}, {user}) => {
        if(!['экспедитор', 'суперэкспедитор'].includes(user.role)) {
            ...
            await Promise.all([
                ...
                InvoiceAzyk.updateMany({_id: {$in: selectedOrders}}, {distributed: false})
            ])
       }
        return 'OK'
   }
