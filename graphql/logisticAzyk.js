...
const resolvers = {
    financeReport: async(parent, {organization, forwarder, dateDelivery, track}, {user}) =>  {
        if(['суперорганизация', 'организация', 'admin', 'экспедитор', 'агент', 'менеджер'].includes(user.role)) {
            ...
            }).select('_id adss createdAt number client agent allPrice rejectedPrice address track info discount paymentMethod inv orders')
                .populate({path: 'adss', select: '_id item count', populate: {path: 'item', select: '_id name unit packaging weight'}})
                .populate({path: 'orders', populate: {path: 'item', select: '_id name packaging'}})
    ...
    summaryAdss: async(parent, {organization, track, agent, forwarder, dateDelivery}, {user}) =>  {
        if(['суперорганизация', 'организация', 'admin', 'агент', 'экспедитор', 'менеджер'].includes(user.role)) {
            ...
            if(user.organization) organization = user.organization
            // eslint-disable-next-line no-undef
            const [invoices, districts] = await Promise.all([
                InvoiceAzyk.find({
                    ...
                    .populate({path: 'adss', select: 'item count', populate: {path : 'item', select: '_id name price packaging weight'}}).lean(),
            ...
            summaryAdss = summaryAdss.map(summaryAds => {
                ...
                for(const parsedSummaryAdsItem of parsedSummaryAdsItems) {
                    ...
                    parsedSummaryAds.push([
                        ...
                        checkFloat(parsedSummaryAdsItem.price*count), checkFloat(parsedSummaryAdsItem.weight*count), summaryAds._id
                    ])
                }
